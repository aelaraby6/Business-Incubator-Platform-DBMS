...
export default function Funding() {
  ...
  const fetchFundingRequests = useCallback(async () => {
    ...
    try {
      ...
      // Handle both array response or object with data array
  ...
  // We are skipping fetchDashboard for now because the React component
  // calculates stats locally using calculateStats above!
  /*
  const fetchDashboard = useCallback(async () => {
    try {
      await invoke("funding:getDashboard");
  ...
  */
  ...
  const filteredRequests = fundingRequests.filter((request) => {
    // Updated to use flat data from our web backend
    const matchesSearch =
    ...
    const matchesStage =
        filterStage === "all" ||
        request.stage?.toLowerCase() === filterStage.toLowerCase();
        ...
        {showDetails ? (
            ...
            <span className="bg-black text-white px-3 py-1 font-bold text-sm uppercase tracking-wider mb-2 inline-block transform -rotate-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
                <StatCard title="Total Requests" value={stats.total} icon={Layers} bgClass="bg-[#4f46e5]" textClass="text-white" />
                <StatCard title="Pending" value={stats.pending} icon={Clock} bgClass="bg-[#f59e0b]" textClass="text-black" />
                <StatCard title="Approved" value={stats.approved} icon={CheckCircle} bgClass="bg-[#0d9488]" textClass="text-white" />
                <StatCard title="Rejected" value={stats.rejected} icon={XCircle} bgClass="bg-[#ef4444]" textClass="text-white" />
                <StatCard title="Total Volume" value={`$${(stats.totalAmount || 0).toLocaleString("en-US", { maximumFractionDigits: 0 })}`} icon={DollarSign} bgClass="bg-[#0f172a]" textClass="text-white" />
              </div>
              ...
              <div className="bg-white p-6 border-4 border-black shadow-[6px_6px_0px_0px_#000] mb-8 flex flex-col xl:flex-row gap-6 items-center justify-between">
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                  <div className="relative w-full sm:w-auto">
                    <Filter className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={18} strokeWidth={2.5} />
                    <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full sm:w-48 pl-10 pr-8 py-3 border-2 border-black bg-white text-black font-bold uppercase outline-none focus:bg-blue-50 cursor-pointer shadow-[2px_2px_0_0_black]">
                  ...
                  <div className="relative w-full sm:w-auto">
                    <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={18} strokeWidth={2.5} />
                    <select value={filterStage} onChange={(e) => setFilterStage(e.target.value)} className="w-full sm:w-48 pl-10 pr-8 py-3 border-2 border-black bg-white text-black font-bold uppercase outline-none focus:bg-blue-50 cursor-pointer shadow-[2px_2px_0_0_black]">
              ...
              {loading ? (
                  <div className="flex flex-col items-center justify-center py-24 bg-white border-4 border-black border-dashed">
                    ...
                    <p className="text-black font-black text-xl uppercase">Loading requests...</p>
                  </div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse min-w-[1000px]">
                        ...
                        <tr className="bg-[#0f172a] border-b-4 border-black text-white">
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Project</th>
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Investor</th>
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Amount</th>
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Stage</th>
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Status</th>
                          <th className="px-6 py-5 font-black uppercase text-sm border-r-2 border-white/20">Date</th>
                          <th className="px-6 py-5 text-center font-black uppercase text-sm">Action</th>
                        </tr>
                        </thead>
                        <tbody className="divide-y-2 divide-black">
                        {filteredRequests.map((request) => (
                            <tr key={request.id} className="hover:bg-blue-50 transition-colors group">
                              ...
                              <td className="px-6 py-5 border-r-2 border-black">
                                <div>
                                  <p className="font-bold text-black uppercase">
                                    {request.investor_name || "Unknown"}
                                  </p>
                                  <p className="text-xs font-bold text-gray-500 lowercase">Investor</p>
                                </div>
                              </td>
                              <td className="px-6 py-5 border-r-2 border-black">
                            ...
                            {request.stage || "N/A"}
                              ...
                              <td className="px-6 py-5 border-r-2 border-black">
                                <div className={getStatusBadgeStyles(request.status)}>
                                  {request.status?.toLowerCase() === "approved" && <CheckCircle size={14} strokeWidth={3} />}
                                  {request.status?.toLowerCase() === "pending" && <Clock size={14} strokeWidth={3} />}
                                  {request.status?.toLowerCase() === "rejected" && <XCircle size={14} strokeWidth={3} />}
                                  {request.status}
                                </div>
                              </td>
                  ...
                  <div className="flex flex-col items-center justify-center py-24 bg-white border-4 border-black border-dashed text-center">
                    <div className="p-6 bg-gray-100 border-2 border-black rounded-full mb-6">
                      <DollarSign className="text-black" size={48} strokeWidth={1.5} />
                    </div>
                    <h2 className="text-2xl font-black text-black uppercase mb-2">No requests found</h2>
                    <p className="text-gray-600 text-lg font-medium max-w-sm mb-8">There are no funding requests matching your criteria.</p>
                  </div>
              )}
            </div>
        )}
